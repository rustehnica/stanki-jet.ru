...
$(window).on("ready", function(event){
	...
	var sendRequestPrice = function(event){
		...
		var comm = $("#requestPriceForm").find('input[type="text"], textarea').val();
		var tel = $("#requestPriceFormTelephone").val();
		var name = $("#requestPriceFormName").val();
		var product = $("#requestPrice .productNameBlock span").text();
		...
		if($requestPriceFormTelephone.val() !="" && $personalInfo.prop("checked")){
			...
			$.getJSON(ajaxPath + "?" + $requestPriceForm.serialize()).done(function(jData){
				...
				let data = {
					fields:
					{ 
						"SOURCE_ID" : "METALK_STANKI_JET_RU",
						"TITLE": "Запрос цены Stanki-JET.ru", 
						"NAME": name, 
						"STATUS_ID": "NEW", 
						"OPENED": "Y", 
						"ASSIGNED_BY_ID": 26, 
						"PHONE": [ { "VALUE": tel, "VALUE_TYPE": "WORK" } ] ,
						"COMMENTS": product + "<br><br>" + comm
					},
					params: { 
	...
	var sendFastBack = function(event){
		...
		if($fastBuyFormName.val() != "" && $fastBuyFormTelephone.val() !="" && $personalInfo.prop("checked")){
			...
			$.getJSON(ajaxPath + "?" + $fastBuyForm.serialize()).done(function(jData){
				...
				var comm = $("#fastBuyFormMessage").val();
				var tel = $("#fastBuyFormTelephone").val();
				var name = $("#fastBuyFormName").val();
				var product = $("#fastBuyName span").text();
				var price = $("#fastBuyPrice").text().replace("/\u00A0/g", '').replace(" руб", "");
				...
				let data = {
					...
					{ 
						"SOURCE_ID" : "METALK_STANKI_JET_RU",
						"TITLE": "Заказ в 1 клик Stanki-JET.ru", 
						"NAME": name, 
						"STATUS_ID": "NEW", 
						"OPENED": "Y", 
						"ASSIGNED_BY_ID": 26, 
						"PHONE": [ { "VALUE": tel, "VALUE_TYPE": "WORK" } ] ,
						"COMMENTS": 'Ссылка на товар: <a href="' + document.location.href + '">' + product + "</a>" + "<br><br>Комментарий: " + comm,
						"OPPORTUNITY": price
					},
					params: { 
						"REGISTER_SONET_EVENT": "Y"											
