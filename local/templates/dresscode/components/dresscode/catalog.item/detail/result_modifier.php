<?
	...
	// получаем акцию из свойств раздела
	$res = CIBlockSection::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID" => $arResult['IBLOCK_ID'], "ID" => $arResult['IBLOCK_SECTION_ID']), false, array("UF_SALE"));
	if($arRes = $res->GetNext()){
		$arResult["SECTION"]["UF_SALE"] = $arRes['~UF_SALE'];
	}unset($res);unset($arRes);

	// получаем металл или дерево
	$nav = CIBlockSection::GetNavChain(false, $arResult["IBLOCK_SECTION_ID"],array());
	while($arRes = $nav->GetNext()){
		if($arRes["ID"] == 1115){
			define("TYPE", "metal");
			$GLOBALS["TYPE"] = "metal";
		}elseif($arRes["ID"] == 1303 || $arRes["ID"] == 1385 || $arRes["ID"] == 1716){
			$GLOBALS["TYPE"] = "wood";
		}
	}unset($nav);unset($arRes);
?>